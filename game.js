...
class Game {
    ...
    async onSwap(candy1, candy2) {
        ...
        const isStable = await this.board.processMatches(false, [candy1, candy2]);

        if (isStable && this.comboCount < 6) {
            // If board is stable (no matches found), swap back, unless in high-combo mode
            await this.board.swapCandies(candy1, candy2);
        }
