...
export default class Board {
    ...
    async activateSpecial(specialCandy) {
        if (!specialCandy || !specialCandy.dataset.powerup) return [];
    
        const affected = new Set();
        const r = parseInt(specialCandy.dataset.row);
        const c = parseInt(specialCandy.dataset.col);
        const powerupType = specialCandy.dataset.powerup;
    
        // The special candy itself is always affected.
        affected.add(specialCandy);
    ...
    async processMatches(isInitializing = false, swappedCandies = null) {
        let matchFoundInLoop = false;
        
        const allCandiesToRemove = new Set();
    
        // Start a loop to handle chain reactions.
        do {
            matchFoundInLoop = false;
            const matchGroups = this.findMatchGroups();
    
            if (matchGroups.length === 0) {
                break; // No more direct matches, exit loop.
            }
            ...
            const currentLoopCandiesToRemove = new Set(totalMatchedCandies);
    
            // Don't remove candies that are becoming powerups
            createdPowerups.forEach(p => {
                const candyToUpgrade = this.grid[p.row][p.col];
                if (candyToUpgrade && currentLoopCandiesToRemove.has(candyToUpgrade)) {
                    currentLoopCandiesToRemove.delete(candyToUpgrade);
            ...
            if (currentLoopCandiesToRemove.size > 0) {
                this.onMatch(Array.from(currentLoopCandiesToRemove), swappedCandies !== null);
                
                const activationQueue = [];
                currentLoopCandiesToRemove.forEach(candy => {
                    allCandiesToRemove.add(candy);
                    if (candy.dataset.powerup) {
                        activationQueue.push(candy);
                    }
                    this.grid[parseInt(candy.dataset.row)][parseInt(candy.dataset.col)] = null;
                });
    
                // Handle chained special activations
                while(activationQueue.length > 0) {
                    const special = activationQueue.shift();
                    await this.pausableTimeout(150); // Delay for visual clarity
    
                    const affectedBySpecial = await this.activateSpecial(special);
                    
                    if (affectedBySpecial.length > 0) {
                        this.onMatch(affectedBySpecial, false); // This is a chain reaction, not a direct player move
    
                        affectedBySpecial.forEach(affectedCandy => {
                            if (!allCandiesToRemove.has(affectedCandy)) {
                                allCandiesToRemove.add(affectedCandy);
                                if (affectedCandy.dataset.powerup) {
                                    activationQueue.push(affectedCandy); // It's another special!
                                }
                                const r = parseInt(affectedCandy.dataset.row);
                                const c = parseInt(affectedCandy.dataset.col);
            ...
            // After resolving a set of matches, let's see if we need to drop and fill before finding the next.
            // This simplifies the logic by handling one "layer" of matches at a time.
            if(allCandiesToRemove.size > 0) {
                allCandiesToRemove.forEach(candy => candy.classList.add('matched'));
                await this.pausableTimeout(300);
                allCandiesToRemove.forEach(candy => candy.remove());
                allCandiesToRemove.clear(); // Clear for next potential iteration
    
                await this.dropCandies();
                await this.fillBoard();
            }
            
            // The loop will continue if new matches were found.
            // Reset swappedCandies after the first pass to avoid re-creating powerups from cascades
            swappedCandies = null; 
    
        } while (matchFoundInLoop);
        
        // Final check for any remaining removals if the loop logic is adjusted
        if (allCandiesToRemove.size > 0) {
            allCandiesToRemove.forEach(candy => candy.classList.add('matched'));
            await this.pausableTimeout(300);
            allCandiesToRemove.forEach(candy => candy.remove());
            await this.dropCandies();
            await this.fillBoard();
            // Recursive call to handle cascades from filling
            await this.processMatches(false, null);
        }
    
        return this.findMatchGroups().length === 0; // Return true if board is stable, false if matches were just cleared.
    }
